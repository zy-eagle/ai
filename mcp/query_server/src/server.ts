...
export async function createServer(): Promise<McpServer> {
  ...
  const server = new McpServer(
    { name: 'query-server', version: '0.1.0' },
    {
      instructions:
        `Authenticated data query server. Auth mode: ${authDescription}. ` +
  ...
  server.registerTool(
    'query_data',
    {
      title: 'Query Data',
      description:
        'Fetch data from the authenticated API. ' +
          ...
          content: [
            {
              type: 'text' as const,
              text: JSON.stringify(
                { status: response.status, data: sanitized },
                null,
                2,
              ),
            },
